...
router.post('/', authenticate, async (req, res, next) => {
  try {
    ...
    const registrationId = await insert('registrations', {
      user_id: req.user.id,
      name,
      gender: gender || null,
      phone,
...
router.put('/:id/status', authenticate, authorize('admin'), async (req, res, next) => {
  try {
    ...
    const registration = await getOne('SELECT id FROM registrations WHERE id = ?', [id])
    if (!registration) {
      return res.status(404).json({ message: '报名记录不存在' })
    }

    await query('UPDATE registrations SET status = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?', [status, id])
