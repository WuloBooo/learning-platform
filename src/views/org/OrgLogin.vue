<template>
  <div class="org-login-page">
    <div class="login-card">
      <h2>机构管理平台</h2>
      <p class="subtitle">请使用机构账号登录</p>

      <div class="form-group">
        <label>账号</label>
        <input v-model="form.username" type="text" placeholder="请输入机构账号" @keyup.enter="handleLogin" />
      </div>
...
const handleLogin = async () => {
  ...
  try {
    const res = await orgAPI.login(form.value)
    if (res.data) {
